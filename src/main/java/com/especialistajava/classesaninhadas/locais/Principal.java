...
public class Principal {
    ...
    private static final NumberFormat FORMATADOR_MONETARIO =
        new DecimalFormat("¤ #,##0.00");

    public static void main(String[] args) {
        Conta conta = new Conta(new BigDecimal("1000"));
        System.out.printf("Saldo inicial: %s%n",
            FORMATADOR_MONETARIO.format(conta.getSaldo()));
        ...
        System.out.printf("Transação 1: %s%n",
            FORMATADOR_MONETARIO.format(transacao1.getValorTotal()));
        System.out.printf("Transação 2: %s%n",
            FORMATADOR_MONETARIO.format(transacao2.getValorTotal()));
